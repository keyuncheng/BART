...
void StripeMergeG::getSolutionForStripeGroup(StripeGroup &stripe_group, vector<vector<size_t> > &solutions, mt19937 random_generator) {
    ...
    for (size_t parity_id = 0; parity_id < code.m_f; parity_id++) {
        ...
        size_t random_pos = Utils::randomUInt(0, min_cost_nodes.size() - 1, random_generator);
        size_t parity_comp_node_id = min_cost_nodes[random_pos];

        // // Strategy 2: pick the first node
        // size_t parity_comp_node_id = min_cost_nodes[0];
        ...
        if (relocated_blk_distribution[parity_comp_node_id] == false) {
            ...
            size_t random_pos = Utils::randomUInt(0, reloc_node_candidates.size() - 1, random_generator);

            // // Strategy 2: pick the first node with minimum cost to compute
            // size_t random_pos = 0;

            size_t parity_reloc_node_id = reloc_node_candidates[random_pos];
    ...
    for (auto &item : data_blocks_to_reloc) {
        ...
        size_t reloc_node_id = reloc_node_candidates[random_pos];
