...
void TransSolution::buildTransTasks(StripeBatch &stripe_batch)
{
    ...
    for (auto &item : stripe_batch.selected_sgs)
    {
        ...
        { // 1. Re-encoding
            ...
            // 1.1 create compute task
            TransTask *compute_task = new TransTask(TransTaskType::COMPUTE_RE_BLK, stripe_group.id, INVALID_BLK_ID, INVALID_STRIPE_ID, INVALID_STRIPE_ID, INVALID_BLK_ID, enc_node, enc_node);
        ...
        { // 2. Parity Merging
            for (uint8_t parity_id = 0; parity_id < code.m_f; parity_id++)
            {
                ...
                for (uint32_t stripe_id = 0; stripe_id < code.lambda_i; stripe_id++)
                {
                    ...
                    // 2.2 create parity block read tasks
        ...
        for (uint32_t stripe_id = 0; stripe_id < code.lambda_i; stripe_id++)
        {
            ...
            for (uint8_t parity_id = 0; parity_id < code.m_f; parity_id++)
            {
                ...
                TransTask *delete_task = new TransTask(TransTaskType::DELETE_BLK, stripe_group.id, INVALID_BLK_ID, stripe->id, stripe_id, code.k_i + parity_id, parity_node_id, parity_node_id);
