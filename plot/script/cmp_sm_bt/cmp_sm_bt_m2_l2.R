# sm_g tasks
 if (T) {
     x1<-read.table("../data/preliminary/cmp_sm_bt_m2_l2.dat", header=TRUE)
     library(ggplot2)
     library(grid)
     library(extrafont)
     font_import()
     ...
     scale_x_discrete(breaks=c("a","b","c","d","e","f","g"),
                      labels=c("2", "3", "4", "5", "6", "8", "16")) +
     scale_y_continuous(expand = c(0,0),limits=c(0,76), breaks=c(0,15,30,45,60,75)) +
     ...
     ggsave("../pdf/cmp_sm_bt_m2_l2.pdf", width=4, height=2.7,device=cairo_pdf)
 }
