# sm_g tasks
 if (T) {
     x1<-read.table("../data/preliminary/cmp_sm_bt_m2_l3.dat", header=TRUE)
     library(ggplot2)
     library(grid)
     library(extrafont)
     font_import()
     ...
     scale_y_continuous(expand = c(0,0),limits=c(0,26), breaks=c(0,5,10,15,20,25)) +
     ...
     ggsave("../pdf/cmp_sm_bt_m2_l3.pdf", width=4, height=2.7,device=cairo_pdf)
 }
