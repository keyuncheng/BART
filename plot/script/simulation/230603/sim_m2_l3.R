# sm_g tasks
 if (T) {
     x1<-read.table("../../../data/simulation/230603/sim_m2_l3.dat", header=TRUE)
     library(ggplot2)
     library(grid)
     library(extrafont)
     font_import()
     ...
     scale_y_continuous(expand = c(0,0),limits=c(0,3100), breaks=c(0,1000,2000,3000)) +
    ...
    #  geom_text(aes(label = max_load), hjust=-2, vjust=-0.4, color = "black", size=2, angle=60, stat="identity") +
     ...
     ggsave("../../../pdf/simulation/230603/sim_m2_l3.pdf", width=4, height=2.7,device=cairo_pdf)
 }
