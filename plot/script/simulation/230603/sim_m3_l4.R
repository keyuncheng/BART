# sm_g tasks
 if (T) {
     x1<-read.table("../../../data/simulation/230603/sim_m3_l4.dat", header=TRUE)
     library(ggplot2)
     library(grid)
     library(extrafont)
     font_import()
     ...
     scale_y_continuous(expand = c(0,0),limits=c(0,2800), breaks=c(0,600,1200,1800,2400)) +
    ...
    #  geom_text(aes(label = max_load), hjust=-2, vjust=-0.4, color = "black", size=2, angle=60, stat="identity") +
     ...
     ggsave("../../../pdf/simulation/230603/sim_m3_l4.pdf", width=4, height=2.7,device=cairo_pdf)
 }
