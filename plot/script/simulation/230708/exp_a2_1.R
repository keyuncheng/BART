# sm_g tasks
if (T) {
    x1<-read.table("../../../data/simulation/230708/exp_a2_1.dat", header=TRUE)
    library(ggplot2)
    library(grid)
    library(extrafont)
    font_import()
    ...
    scale_x_discrete(breaks=c("a","b","c","d","e"),
                    labels=c("(6,2)", "(6,3)", "(10,2)", "(10,4)", "(16,4)")) +
    ...
    xlab(expression(paste("(k,m)"))) +
    theme_classic() +
    # theme(axis.text.x = element_text(size=20, angle=30, hjust=0.8, vjust=0.8, color="black", family="Times New Roman")) +
    theme(axis.text.x = element_text(size=20, angle=0, hjust=0.5, vjust=0.5, color="black", family="Times New Roman")) +
    ...
    # theme(border=F)
    ggsave("../../../pdf/simulation/230708/exp_a2_1.pdf", width=6, height=4.4, device=cairo_pdf)
}
