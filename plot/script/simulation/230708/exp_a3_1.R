if (T) {
    x1<-read.table("../../../data/simulation/230708/exp_a3_1.dat", header=TRUE)
    library(ggplot2)
    library(grid)
    library(extrafont)
    font_import()
    ...
    scale_y_continuous(expand = c(0,0),limits=c(0,900), breaks=c(0,200,400,600,800)) +
    ...
    theme(legend.position=c(0.2,0.8), legend.direction = "horizontal", legend.key=element_blank())
    ggsave("../../../pdf/simulation/230708/exp_a3_1.pdf", width=5.5, height=4, device=cairo_pdf)
}