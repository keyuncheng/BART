...
public class StripedBlockUtil {
  ...
  public static LocatedBlock[] parseStripedBlockGroupWithTransition(LocatedStripedBlock bg,
      int cellSize, int dataBlkNum, int parityBlkNum) {
    // block group length
    // For example: 2*(3, 2) -> (6, 2)
    // The newly generate (6, 2) actual BGsize: 8; logical BGsize: 5.

    int locatedBGSize = bg.getBlockIndices().length; 
    int locatedLogicalBGSize = locatedBGSize;
    
    for (short i = 0; i < locatedBGSize; i++) {
      final int idx = bg.getBlockIndices()[i];
      if (idx == -1) {
        locatedLogicalBGSize--;
      }
    }
    dataBlkNum = locatedLogicalBGSize - parityBlkNum;


    // Initialize the LocatedBlock list
    // Where it only contains the block with InfoUseful = true
    LocatedBlock[] lbs = new LocatedBlock[locatedLogicalBGSize];
    for (short i = 0; i < locatedBGSize; i++) {
      final int idx = bg.getBlockIndices()[i];
      if (idx == -1) {
        continue;
  ...
  public static LocatedBlock constructInternalBlock(LocatedStripedBlock bg,
    ...
    if (bg.getLocations()[idxInReturnedLocs].getPoolID() == null) {
      blk = constructInternalBlock(
          bg.getBlock(), cellSize, dataBlkNum, idxInBlockGroup);
    } else {
      blk = constructInternalBlock(
          bg.getBlock(), cellSize, dataBlkNum, idxInBlockGroup,
          bg.getLocations()[idxInReturnedLocs].getPoolID(),
          bg.getLocations()[idxInReturnedLocs].getBlockID(),
          bg.getLocations()[idxInReturnedLocs].getGenStamp());
    }
  ...
  /**
   * This duplicate method creates an internal {@link ExtendedBlock} with the block
   * original block id instead of the offset in the block group.
   */
  public static ExtendedBlock constructInternalBlock(
      ExtendedBlock blockGroup, int cellSize, int dataBlkNum,
      int idxInBlockGroup, String poolId, long blockId, long genStamp) {
    ExtendedBlock block = new ExtendedBlock(blockGroup);
  ...
  public static AlignedStripe[] divideOneStripe(ErasureCodingPolicy ecPolicy,
    ...
    StripingCell[] cells = getStripingCellsOfByteRange(ecPolicy, cellSize,
        blockGroup, rangeStartInBlockGroup, rangeEndInBlockGroup, ecPolicy.getNumDataUnits());
  ...
  public static AlignedStripe[] divideOneStripe(ErasureCodingPolicy ecPolicy,
      int cellSize, LocatedStripedBlock blockGroup, long rangeStartInBlockGroup,
      long rangeEndInBlockGroup, ByteBuffer buf, int logicalDataBlkNum) {
    // Step 1: map the byte range to StripingCells
    StripingCell[] cells = getStripingCellsOfByteRange(ecPolicy, cellSize,
        blockGroup, rangeStartInBlockGroup, rangeEndInBlockGroup, logicalDataBlkNum);
    ...
    // Step 4: calculate each chunk's position in destination buffer. Since the
    // whole read range is within a single stripe, the logic is simpler here.
    int bufOffset = (int) (rangeStartInBlockGroup % ((long) cellSize * logicalDataBlkNum));
    for (StripingCell cell : cells) {
      long cellStart = cell.idxInInternalBlk * cellSize + cell.offset;
      long cellEnd = cellStart + cell.size - 1;
      for (AlignedStripe s : stripes) {
    ...
    // Step 5: prepare ALLZERO blocks
    prepareAllZeroChunks(blockGroup, stripes, cellSize, logicalDataBlkNum);
    return stripes;
  }
  ...
  private static StripingCell[] getStripingCellsOfByteRange(
    ...
    cells[0] = new StripingCell(ecPolicy, firstCellSize, firstCellIdxInBG,
        firstCellOffset, logicalDataBlkNum);
    if (lastCellIdxInBG != firstCellIdxInBG) {
      ...
      cells[numCells - 1] = new StripingCell(ecPolicy, lastCellSize,
          lastCellIdxInBG, 0, logicalDataBlkNum);
    }

    for (int i = 1; i < numCells - 1; i++) {
      cells[i] = new StripingCell(ecPolicy, cellSize, i + firstCellIdxInBG, 0, logicalDataBlkNum);
    }
  ...
  public static class StripingCell {
    ...
    StripingCell(ErasureCodingPolicy ecPolicy, int cellSize, long idxInBlkGroup,
        long offset, int logicalNumData) {
      this.ecPolicy = ecPolicy;
      this.idxInBlkGroup = idxInBlkGroup;
      this.idxInInternalBlk = idxInBlkGroup / logicalNumData;
      this.idxInStripe = (int) (idxInBlkGroup -
          this.idxInInternalBlk * logicalNumData);
      this.offset = offset;
      this.size = cellSize;
    }
