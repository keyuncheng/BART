...
public class PBHelperClient {
  ...
  public static LocatedBlock convertLocatedBlockProto(LocatedBlockProto proto) {
    ...
    String[] poolIds = new String[proto.getPoolIDsCount()];
    for (int i=0; i<proto.getPoolIDsCount(); i++) {
      poolIds[i] = proto.getPoolIDs(i);
    }
    long[] blockIds = new long[proto.getBlockIDsCount()];
    long[] genStamps = new long[proto.getGenStampsCount()];
    for (int i=0; i<proto.getBlockIDsCount(); i++) {
      blockIds[i] = proto.getBlockIDs(i);
      genStamps[i] = proto.getGenStamps(i);
    }
    ...
    if (indices == null) {      
      ...
      if (proto.getPoolIDsCount() > 0) {
        lb = new LocatedStripedBlock(PBHelperClient.convert(proto.getB()),
          targets, storageIDs, storageTypes, poolIds, blockIds, genStamps,
          indices, proto.getOffset(), proto.getCorrupt(), 
          cachedLocs.toArray(new DatanodeInfo[cachedLocs.size()]));
      } else {
        lb = new LocatedStripedBlock(PBHelperClient.convert(proto.getB()),
  ...
  public static LocatedBlockProto convertLocatedBlock(LocatedBlock b) {
    ...
    final String[] poolIDs = b.getPoolIDs();
    if (poolIDs != null) {
      builder.addAllPoolIDs(Arrays.asList(poolIDs));
    }
    final long[] blockIDsPrimitive = b.getBlockIDs();
    final long[] genStampsPrimivie = b.getGenStamps();
    if (poolIDs != null && blockIDsPrimitive.length != 0 && genStampsPrimivie.length != 0) {  
      Long[] blockIDs = new Long[blockIDsPrimitive.length];
      Long[] genStamps = new Long[genStampsPrimivie.length];
      for (int i=0; i<blockIDsPrimitive.length; i++) {
        blockIDs[i] = new Long(blockIDsPrimitive[i]);
      }
      for (int i=0; i<genStampsPrimivie.length; i++) {
        genStamps[i] = new Long(genStampsPrimivie[i]);
      }
      builder.addAllBlockIDs(Arrays.asList(blockIDs));
      builder.addAllGenStamps(Arrays.asList(genStamps));
    }
