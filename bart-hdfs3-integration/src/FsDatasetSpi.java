...
public interface FsDatasetSpi<V extends FsVolumeSpi> extends FSDatasetMBean {
  ...
  /**
   * Returns an input stream at specified offset for a new block.
   * 
   * @param b          block
   * @param seekOffset offset with in the block to seek to
   * @return an input stream to read the contents of the specified block,
   ...
   * @throws IOException
   */
  InputStream getBlockInputStreamWithNewBlock(ExtendedBlock b, long seekOffset)
      throws IOException;

  /**
   * Returns an input stream at specified offset for a new block.
   * 
   * @param blockFile  block file in local directory
   * @return an input stream to read the contents of the specified block,
   *         starting at the offset
   * @throws IOException
   */
  InputStream getBlockInputStreamWithFile(File blockFile)
      throws IOException;
}
