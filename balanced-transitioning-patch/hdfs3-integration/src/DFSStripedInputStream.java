...
public class DFSStripedInputStream extends DFSInputStream {
  ...
  private final short logicalDataBlkNum;
  ...
  DFSStripedInputStream(DFSClient dfsClient, String src,
    ...
    short logicalCounter = dataBlkNum;
    if (this.locatedBlocks.getLastLocatedBlock() instanceof LocatedStripedBlock) {
      LocatedStripedBlock lsb = (LocatedStripedBlock) this.locatedBlocks.getLastLocatedBlock();
      for (int i=0; i<lsb.getBlockIndices().length; i++) {
        if (lsb.getBlockIndices()[i] == -1) {
          logicalCounter--;
        }
      }
    }
    this.logicalDataBlkNum = logicalCounter;
  ...
  private void readOneStripe(CorruptedBlocks corruptedBlocks)
    ...
    // the stripe statistics are calculated based on the logical data block num
    final long offsetInBlockGroup = getOffsetInBlockGroup();
    final long stripeLen = cellSize * logicalDataBlkNum;
    ...
    AlignedStripe[] stripes = StripedBlockUtil.divideOneStripe(ecPolicy,
        ...
        offsetInBlockGroup + stripeRange.getLength() - 1,
        curStripeBuf, logicalDataBlkNum);

    final LocatedBlock[] blks = StripedBlockUtil.parseStripedBlockGroupWithTransition(
        blockGroup, cellSize, dataBlkNum, parityBlkNum);
    this.locatedBlocks.setUsefulBlocks(blks);

    // read the whole stripe
    for (AlignedStripe stripe : stripes) {
      // Parse group to get chosen DN location
      StripeReader sreader = new TransitionedStripeReader(stripe, ecPolicy, blks,
