...
import org.apache.hadoop.hdfs.protocol.proto.CoordinatorProtocolProtos.CoordinatorProtocolService;
...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  /** The RPC server that listens to requests from coordinator */
  // private final RPC.Server coordinatorRpcServer;
  // private final InetSocketAddress coordinatorRpcAddress;
  ...
  private LocatedBlocks getBlockLocationsFromJSON(String clientMachine, String src, long offset, long length) {
    NewMetaFile currentFile = parseJsonMetadata(src);
    LocatedBlocks lbs = namesystem.getBlockLocations(clientMachine, currentFile);
    return lbs;
  }

  private NewMetaFile parseJsonMetadata(String src) {
    NewMetaFile currentMetaFile = new NewMetaFile();
    try {
      ...
      currentMetaFile.setBlockGroupEcPolicy(jsonData.get("block_group_ec_policy").asText());
      ...
      for (int i=0; i<blockGroupListLength; i++) {
        NewMetaBlockGroup currentMetaBlockGroup  = new NewMetaBlockGroup();
        currentMetaBlockGroup.setBlockGroupName(jsonData.get("block_group_list").get(i).get("block_group_name").asText());
        currentMetaBlockGroup.setBlockGroupSize(jsonData.get("block_group_list").get(i).get("block_group_size").asLong());

        NewMetaBlock[] currentMetaBlocks = new NewMetaBlock[blkNum];
        for (int j=0; j<blkNum; j++) {
          currentMetaBlocks[j] = new NewMetaBlock();
          currentMetaBlocks[j].setBlockDn(jsonData.get("block_group_list").get(i).get("block_list").get(j).get("block_dn").asText());
          currentMetaBlocks[j].setBlockId(jsonData.get("block_group_list").get(i).get("block_list").get(j).get("block_id").asLong());
          currentMetaBlocks[j].setBlockPath(jsonData.get("block_group_list").get(i).get("block_list").get(j).get("block_path").asText());
          currentMetaBlocks[j].setBlockInUse(jsonData.get("block_group_list").get(i).get("block_list").get(j).get("block_in_use").asBoolean());
          currentMetaBlocks[j].setBlockGenStamp(jsonData.get("block_group_list").get(i).get("block_list").get(j).get("block_gen_stamp").asLong());
        }

        currentMetaBlockGroup.setBlockList(currentMetaBlocks);
        metaBlockGroupList[i] = currentMetaBlockGroup;
      }
      currentMetaFile.setBlockGroupList(metaBlockGroupList);
    } catch (IOException e) {
      e.printStackTrace();
    }
...
class NewMetaBlock {
  @JsonProperty("block_dn")
  private String block_dn;

  @JsonProperty("block_id")
  private long block_id;

  @JsonProperty("block_path")
  private String block_path;

  @JsonProperty("block_in_use")
  private boolean block_in_use;

  @JsonProperty("block_gen_stamp")
  private long block_gen_stamp;

  // setters
  public void setBlockDn(String block_dn) {
    this.block_dn = block_dn;
  }
  ...
  public void setBlockPath(String block_path) {
    this.block_path = block_path;
  }

  public void setBlockInUse(boolean inuse) {
    this.block_in_use = inuse;
  }

  public void setBlockGenStamp(long gen_stamp) {
    this.block_gen_stamp = gen_stamp;
  }

  // getters
  public String getBlockDn() {
    return block_dn;
  }
  ...
  public boolean getBlockInUse() {
    return this.block_in_use;
  }

  public long getBlockGenStamp() {
    return this.block_gen_stamp;
  }

  // toString
  public String toString() {
    return "Block datanode: " + block_dn + ", block id: " + block_id + ", block path: " + block_path
        + ", block in use: " + block_in_use + ", block_gen_stamp: " + block_gen_stamp;
  }
}

class NewMetaBlockGroup {
  @JsonProperty("block_group_name")
  private String block_group_name;

  @JsonProperty("block_group_size")
  private long block_group_size;

  private NewMetaBlock[] block_list;

  // setters
  public void setBlockGroupName(String block_group_name) {
    this.block_group_name = block_group_name;
  }

  public void setBlockGroupSize(long block_group_size) {
    this.block_group_size = block_group_size;
  }

  public void setBlockList(NewMetaBlock[] list) {
    this.block_list = list;
  }

  // getters
  public String getBlockGroupName() {
    return this.block_group_name;
  }

  public long getBlockGroupSize() {
    return this.block_group_size;
  }

  public NewMetaBlock[] getBlockList() {
    return block_list;
  }

  public NewMetaBlock getBlockList(int index) {
    return block_list[index];
  }

  // toString
  public String toString() {
    String outputString = "Block group name: " + block_group_name + ", block group size: " + block_group_size + ", block list: \n";
    for (int i=0; i<block_list.length; i++) {
      outputString += "\t" + block_list[i].toString() + "\n";
    }
    return outputString;
  }
}
...
class NewMetaFile {
  ...
  @JsonProperty("block_group_ec_policy")
  private String bg_ec_policy;

  private NewMetaBlockGroup[] block_group_list;
  ...
  public void setBlockGroupEcPolicy(String ecPolicy) {
    this.bg_ec_policy = ecPolicy;
  }

  public void setBlockGroupList(NewMetaBlockGroup[] list) {
    this.block_group_list = list;
  }

  public void setBlockGroupList(int index, NewMetaBlockGroup bg) {
    this.block_group_list[index] = bg;
  }
  ...
  public String getBlockGroupECPolicy() {
    return this.bg_ec_policy;
  }

  public NewMetaBlockGroup[] getBlockGroupList() {
    return this.block_group_list;
  }

  public NewMetaBlockGroup getBlockGroupList(int index) {
    return this.block_group_list[index];
  }

  // toString
  public String toString() {
    String outputString = "File name: " + file_name + ", file size: " + file_size + ", block_group_ec_policy: " + bg_ec_policy + "\n";
    for (int i=0; i<block_group_list.length; i++) {
      outputString += "\t" + block_group_list[i].toString() + "\n";
    }
    return outputString;
  }
}