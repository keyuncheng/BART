...
public class LocatedBlock {
  ...
  public LocatedBlock(ExtendedBlock b, DatanodeInfo[] locs,
      String[] storageIDs, StorageType[] storageTypes, long startOffset,
      boolean corrupt, DatanodeInfo[] cachedLocs, String[] poolIDs,
      long[] blockIDs, long[] genStamps) {
    this(b, convert(locs, storageIDs, storageTypes, poolIDs, blockIDs, genStamps),
        storageIDs, storageTypes, poolIDs, blockIDs, genStamps, startOffset, corrupt,
        null == cachedLocs || 0 == cachedLocs.length ? EMPTY_LOCS : cachedLocs);
  }
  ...
  public LocatedBlock(ExtendedBlock b, DatanodeInfoWithStorage[] locs,
      String[] storageIDs, StorageType[] storageTypes, String[] poolIDs,
      long[] blockIDs, long[] genStamps, long startOffset,
      boolean corrupt, DatanodeInfo[] cachedLocs) {
    this.b = b;
    this.offset = startOffset;
    this.corrupt = corrupt;
  ...
  private static DatanodeInfoWithStorage[] convert(
    DatanodeInfo[] infos, String[] storageIDs,
    StorageType[] storageTypes, String[] poolIDs,
    long[] blockIDs, long[] genStamps) {
    if (null == infos) {
      return EMPTY_LOCS;
    }
    ...
    for (int i = 0; i < infos.length; i++) {
      ret[i] = new DatanodeInfoWithStorage(infos[i],
          storageIDs != null ? storageIDs[i] : null,
          storageTypes != null ? storageTypes[i] : null,
          poolIDs != null ? poolIDs[i] : null,
          blockIDs != null ? blockIDs[i] : 0,
          genStamps != null ? genStamps[i] : 0);
    }
    return ret;
  }
  ...
  public long[] getGenStamps() {
    return genStamps;
  }

  public boolean isPoolIDsEmpty() {
    return poolIDs == null;
  }

  public boolean isBlockIDsEmpty() {
    return blockIDs == null;
  }

  public boolean isGenStampsEmpty() {
    return genStamps == null;
  }
  ...
  public String toString() {
    String returnString = getClass().getSimpleName() + "{" + b
    ...
    if (isBlockIDsEmpty()) {
      ...
      returnString += "; blockIds=" + Arrays.asList(getBlockIDs())
          + "; poolIds=" + Arrays.asList(getPoolIDs())
          + "; genStamps=" + Arrays.asList(getGenStamps())
          + "}";
    }
    return returnString;
  }
