# sm_g tasks
 if (T) {
     x1<-read.table("../data/sm_g_vary_nodes.dat", header=TRUE)
     library(ggplot2)
     library(grid)
     library(extrafont)
     font_import()
     ...
     ggplot(x1, aes(x=num_nodes_id, y=avg_num_send_tasks, fill=ec_param_id)) +
     scale_fill_manual(name=element_blank(),
                       breaks=c("a","b"),
                       labels=c("(9,6)","(14,10)"),
                       #values=c(b='#3366ff',c='#33cccc',d='#999966')) +
                       values=c(a="turquoise2",b="tan")) +
     scale_x_discrete(breaks=c("b","c","d","e"),
                      labels=c("2(2k+m)", "3(2k+m)", "4(2k+m)", "5(2k+m)")) +
     ...
     theme(legend.position=c(0.8,0.85), legend.direction="vertical", legend.key=element_blank())
     ggsave("../pdf/sm_g_vary_nodes.pdf", width=4, height=2.7,device=cairo_pdf)
 }
